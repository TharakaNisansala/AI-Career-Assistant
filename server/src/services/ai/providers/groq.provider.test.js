...
const { complete } = require("./groq.provider");

// Stands in for Groq's OpenAI-compatible API so these tests exercise real
// HTTP end to end (matching this codebase's preference for real I/O over
// mocks) without needing network access or a real API key.
...
test("complete sends the configured model and credentials to the provider", async () => {
  ...
  assert.equal(capturedRequest.payload.model, "llama-3.1-8b-instant");
  assert.deepEqual(capturedRequest.payload.messages, [
